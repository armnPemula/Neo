        ...
        if stager_type == 'cross_platform':
            ...
            try:
                ...
                if stager_type == 'powershell_dropper':
                    stager_content = _generate_encoded_powershell_dropper(listener_info)
                    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"powershell_stager_{timestamp}.txt"
                    filepath = os.path.join("logs", filename)
                    os.makedirs("logs", exist_ok=True)
                    with open(filepath, 'w') as f:
                        f.write(stager_content)
                    response = f"PowerShell stager saved to: {filepath}\n\n{stager_content}"
                    return response, 'success'
                elif stager_type == 'bash_dropper':
                    # Autosave
                    stager_content = _generate_encoded_bash_dropper(listener_info)
                    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"bash_stager_{timestamp}.txt"
                    filepath = os.path.join("logs", filename)
                    os.makedirs("logs", exist_ok=True)
                    with open(filepath, 'w') as f:
                        f.write(stager_content)
                    response = f"Bash stager saved to: {filepath}\n\n{stager_content}"
                    return response, 'success'
            except Exception as e:
